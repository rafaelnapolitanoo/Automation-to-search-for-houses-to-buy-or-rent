...
def tratar_link(link):
    # Seu código de tratamento de link...
    #?qnt quartos 
...
largura_janela = 800  # Reduzir em 100 pixels
altura_janela = 600  # Reduzir em 100 pixels
...
# Criar widgets
tk.Label(janela, text="Localização:", bg="#000000", fg="white", font=fonte).grid(row=0, column=0)
entrada_localizacao = tk.Entry(janela, bg="#333333", fg="white", font=fonte)
entrada_localizacao.grid(row=0, column=1)

tk.Label(janela, text="Quantidade de Quartos:", bg="#000000", fg="white", font=fonte).grid(row=1, column=0)
entrada_quartos = tk.Entry(janela, bg="#333333", fg="white", font=fonte)
entrada_quartos.grid(row=1, column=1)

tk.Label(janela, text="Preço Máximo:", bg="#000000", fg="white", font=fonte).grid(row=2, column=0)
entrada_preco = tk.Entry(janela, bg="#333333", fg="white", font=fonte)
entrada_preco.grid(row=2, column=1)

tk.Label(janela, text="Tipo (Alugar/Comprar):", bg="#000000", fg="white", font=fonte).grid(row=3, column=0)
var_tipo = tk.StringVar(value="ALUGAR")
tk.Radiobutton(janela, text="Alugar", variable=var_tipo, value="ALUGAR", bg="#000000", fg="white", selectcolor="#333333", font=fonte).grid(row=3, column=1)
tk.Radiobutton(janela, text="Comprar", variable=var_tipo, value="COMPRAR", bg="#000000", fg="white", selectcolor="#333333", font=fonte).grid(row=3, column=2)

tk.Label(janela, text="Tipo de Imóvel:", bg="#000000", fg="white", font=fonte).grid(row=4, column=0)
var_tipo_imovel = tk.StringVar(value="APARTAMENTO")
tk.Radiobutton(janela, text="Apartamento", variable=var_tipo_imovel, value="APARTAMENTO", bg="#000000", fg="white", selectcolor="#333333", font=fonte).grid(row=4, column=1)
tk.Radiobutton(janela, text="Casa", variable=var_tipo_imovel, value="CASA", bg="#000000", fg="white", selectcolor="#333333", font=fonte).grid(row=4, column=2)

tk.Label(janela, text="Quantidade de Garagens:", bg="#000000", fg="white", font=fonte).grid(row=5, column=0)
entrada_garagem = tk.Entry(janela, bg="#333333", fg="white", font=fonte)
entrada_garagem.grid(row=5, column=1)

tk.Label(janela, text="Área Mínima (m²):", bg="#000000", fg="white", font=fonte).grid(row=6, column=0)
entrada_m2 = tk.Entry(janela, bg="#333333", fg="white", font=fonte)
entrada_m2.grid(row=6, column=1)

tk.Button(janela, text="Buscar Imóveis", command=executar_script, bg="#000000", fg="white", font=fonte).grid(row=7, column=0, columnspan=3)

# Listbox para exibir os resultados
lista_resultados = tk.Listbox(janela, bg="#333333", fg="white", width=200, height=40, font=fonte)
